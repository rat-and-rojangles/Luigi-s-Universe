...
class Crashman(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y, level, character):
        ...
        #crashman images
        self.run_left = []
        for ipath in os.listdir( "characters/"+self.character+"/run_left" ):
            self.run_left.append(pygame.image.load("characters/"+self.character+"/run_left/"+ipath).convert_alpha())
            
        self.run_right = []
        for ipath in os.listdir( "characters/"+self.character+"/run_right" ):
            self.run_right.append(pygame.image.load("characters/"+self.character+"/run_right/"+ipath).convert_alpha())

        self.jump_left=pygame.image.load("characters/"+self.character+"/jump_left.png").convert_alpha()
        self.jump_right=pygame.image.load("characters/"+self.character+"/jump_right.png").convert_alpha()
        
        self.down_right=pygame.image.load("characters/"+self.character+"/down_right.png").convert_alpha()
        self.down_left=pygame.image.load("characters/"+self.character+"/down_left.png").convert_alpha()

        self.idle_right=pygame.image.load("characters/"+self.character+"/idle_right.png").convert_alpha()
        self.idle_left=pygame.image.load("characters/"+self.character+"/idle_left.png").convert_alpha()
        ...
        for o in self.world:
            if self.rect.colliderect(o) and (o.solid==True and o.type!="enemy") and o!=self:
                ...
                if movy < 0:
                    ...
                    if o.type=="block":
                        o.bumping=True
            elif self.rect.colliderect(o) and o.type=="flag":
                self.foundFlag=True
